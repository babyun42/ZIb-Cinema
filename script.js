    ...
    tailwind.config = {
        ...
        theme: {
            extend: {
                colors: {
                    m3: {
                        surface: withOpacity('--m3-surface'),
                        surfaceDim: withOpacity('--m3-surface-dim'),
                        surfaceBright: withOpacity('--m3-surface-bright'),
                        surfaceContainerLowest: withOpacity('--m3-surface-container-lowest'),
                        surfaceContainerLow: withOpacity('--m3-surface-container-low'),
                        surfaceContainer: withOpacity('--m3-surface-container'),
                        surfaceContainerHigh: withOpacity('--m3-surface-container-high'),
                        surfaceContainerHighest: withOpacity('--m3-surface-container-highest'),
                        primary: withOpacity('--m3-primary'),
                        onPrimary: withOpacity('--m3-on-primary'),
                        primaryContainer: withOpacity('--m3-primary-container'),
                        onPrimaryContainer: withOpacity('--m3-on-primary-container'),
                        secondary: withOpacity('--m3-secondary'),
                        onSecondary: withOpacity('--m3-on-secondary'),
                        outline: withOpacity('--m3-outline'),
                        outlineVariant: withOpacity('--m3-outline-variant'),
                        onSurface: withOpacity('--m3-on-surface')
                    }
                },
                fontFamily: {
...
const THEME_NEUTRALS = {
    dark: {
        surface: '20 18 24', surfaceDim: '20 18 24', surfaceBright: '59 56 62',
        surfaceContainerLowest: '15 13 19', surfaceContainerLow: '29 27 32',
        surfaceContainer: '33 31 38', surfaceContainerHigh: '43 41 48', surfaceContainerHighest: '54 52 59',
        onSurface: '255 255 255', secondary: '204 194 220', onSecondary: '51 45 65',
        outline: '147 143 153', outlineVariant: '73 69 79'
    },
    light: {
        surface: '254 247 255', surfaceDim: '222 216 225', surfaceBright: '254 247 255',
        surfaceContainerLowest: '255 255 255', surfaceContainerLow: '247 242 250',
        surfaceContainer: '243 237 247', surfaceContainerHigh: '236 230 240', surfaceContainerHighest: '230 224 233',
        onSurface: '29 27 32', secondary: '98 91 113', onSecondary: '255 255 255',
        outline: '121 116 126', outlineVariant: '196 199 197'
    }
};
...
function switchNavTab(tabName) {
    ...
    if(playerView) hideMainView(playerView);
    if(genericView) hideMainView(genericView);
    if(settingsView) hideMainView(settingsView);
    ...
    if (tabName === 'home') {
        ...
        if(settingsView) showMainView(settingsView, direction);
    } else {
...
function updatePopularCountLabel() {
    ...
    const shownCount = grid.querySelectorAll('.group.relative').length;
    countLabel.innerHTML = `Найдено позиций: ${shownCount}`;
}
...
function renderLoadMoreButton() {
    let wrap = document.getElementById('popular-load-more-wrap');
    if (popularTotalPages && popularNextPage > popularTotalPages) {
        if (wrap) wrap.style.display = 'none';
        return;
    }
    ...
    if (!wrap) {
        ...
        if (btn) {
            btn.disabled = false;
            btn.innerHTML = `<i class="fa-solid fa-plus"></i> Ещё`;
        }
    }
}
...
async function loadPopularTop100() {
    ...
    grid.innerHTML = `<div class="col-span-full text-center py-10"><i class="fa-solid fa-spinner fa-spin text-m3-primary text-2xl"></i><span class="ml-3 text-m3-outline text-sm block mt-2">Загрузка рейтинга...</span></div>`;
...
async function loadMorePopular() {
    ...
    try {
        ...
        if (items.length > 0) {
            ...
            tempDiv.innerHTML = items.map(movie => createMovieCard(movie, false)).join('');
            while (tempDiv.firstChild) {
                grid.insertBefore(tempDiv.firstChild, document.getElementById('popular-load-more-wrap'));
            }
